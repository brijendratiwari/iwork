...
static const int portraitItemHeight = 90;
...
- (id)initWithFrame:(CGRect)frame 
{
    if ((self = [super initWithFrame:frame])) 
	{
       ...
       // pagesScrollView.backgroundColor = [UIColor greenColor];
        ...
        if (IS_IPAD) {
             pageControl = [[MyLauncherPageControl alloc] initWithFrame:CGRectMake(0, 515, frame.size.width, pControllHeight)];
        }else{
            if (IS_IPHONE_5) {
                pageControl = [[MyLauncherPageControl alloc] initWithFrame:CGRectMake(0, 250, frame.size.width, pControllHeight)];
            }else{
                pageControl = [[MyLauncherPageControl alloc] initWithFrame:CGRectMake(0, 145, frame.size.width, pControllHeight)];
            }
...
-(void)itemTouchedUpInside:(MyLauncherItem *)item
{
	...
	else 
	{
        ...
        [[self delegate] launcherItemSelected:item];
		pagesScrollView.scrollEnabled = YES;
	}
}
...
-(void)itemTouchedDown:(MyLauncherItem *)item
{
	...
	else 
	{
		...
		itemHoldTimer = [NSTimer scheduledTimerWithTimeInterval:2 target:self selector:@selector(itemHoldTimer:) userInfo:item repeats:NO];
	}
}
...
-(void)savePages
{
	...
	for(NSArray *page in pages)
	{
		...
		for(MyLauncherItem *item in page)
		{
            ...
            NSLog(@"set tag - %ld",(long)item.itemTag);
            [itemToSave setObject:[NSString stringWithFormat:@"%d", [item itemTag]] forKey:@"tag"];
