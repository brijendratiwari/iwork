...
@synthesize delegate, targetController, title, image, closeButton, controllerStr, itemTag;

-(id)initWithTitle:(NSString *)_title image:(NSString *)_image target:(NSString *)_controllerStr deletable:(BOOL)_deletable tag:(int)tag
{
	if((self = [super init]))
	{ 
		dragging = NO;
...
-(void)layoutItem
{
    ...
    itemLabel.font = [UIFont fontWithName:@"Arial" size:8.0f];
    if (IS_IPAD) {
        itemLabel.font = [UIFont fontWithName:@"Arial" size:18.0f];
    }
	
	itemLabel.textColor = [UIColor colorWithRed:104/255 green:104/255 blue:104/255 alpha:1.0];
